...
from tkinter import Tk, Label, Entry, Button, PhotoImage, messagebox as mb
...
def login_window():
    login = tk.Tk()
    login.title('Login')
    login.geometry("1100x600")
    login.call('source', 'Azure/azure.tcl')
    login.call('set_theme', 'dark')
    ...
    # Font settings for labels, entry fields, and buttons
    label_font = ("Arial", 16)
    entry_font = ("Arial", 14)
    button_font = ("Arial", 14)

    tk.Label(login, text="Username:", font=label_font).pack(pady=20)
    username_entry = ttk.Entry(login, font=entry_font)  # Use ttk.Entry
    username_entry.pack(pady=10)

    Label(login, text="Password:", font=label_font).pack(pady=20)
    password_entry = ttk.Entry(login, show="*", font=entry_font)  # Use ttk.Entry
    ...
    # Use a ttk button
    ttk.Button(login, text="Login", command=validate_login, style="TButton").pack(pady=30)
