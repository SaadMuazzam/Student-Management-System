...
def check_credentials():
    if userentry.get() == "admin" and passentry.get() == "password":
        ...
        error_label = ctk.CTkLabel(frame, text="Incorrect username \n or password",font=("Arial",12,), text_color="Black")
        error_label.grid(row=5, column=0, padx=40, pady=10)

mybutton = ctk.CTkButton(frame, text="Login", command=check_credentials) # Assign check_credentials function to the button click

username.grid(row=0, column=0, padx=40, pady=20)  # Add padding for spacing
userentry.grid(row=1, column=0, padx=40, pady=5)
password.grid(row=2, column=0, padx=40, pady=5)
passentry.grid(row=3, column=0, padx=40, pady=5)
mybutton.grid(row=4, column=0, columnspan=2, padx=40, pady=40)  # Span both columns for the button
