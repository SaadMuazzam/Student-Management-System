...
def login_window():
    login = Tk()
    login.title('Login')
    login.attributes('-fullscreen', True)  # Make the login page fullscreen
    #login.resizable(0, 0)

    # Font settings for labels, entry fields, and buttons
    label_font = ("Segoe", 16)
    entry_font = ("Segoe", 14)
    button_font = ("Segoe", 14)

    Label(login, text="Username:", font=label_font).pack(pady=20)
    username_entry = Entry(login, font=entry_font)
    username_entry.pack(pady=10)

    Label(login, text="Password:", font=label_font).pack(pady=20)
    password_entry = Entry(login, show="*", font=entry_font)
    ...
    Button(login, text="Login", command=validate_login, font=button_font).pack(pady=30)
